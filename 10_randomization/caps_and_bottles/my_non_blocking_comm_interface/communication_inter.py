...
class Communication:
    ...
    def __init__(self, child_script: str):
        ...
        self.output_lines = Queue()

        # Start a thread to read stdout
        self.thread = threading.Thread(target=self._enqueue_output, args=(self.proc.stdout, self.output_lines))
    ...
    def read(self) -> str|None:
        ...
        while not self.isfinish():
            try:
                line = self.output_lines.get_nowait()  # Non-blocking get
            except Empty:
                time.sleep(self.delay)  # Small delay before retrying
